...
function appGender() {
  ...
  $(".chat-content-list").append(
    '<div class="chat-content-buttons-gender"><div class="chat-content-buttons-gender-block"><button class="chooseGenderM" data-gender="Мужчина">Мужчина</button></div><div class="chat-content-buttons-gender-block"><button class="chooseGenderW" data-gender="Женщина">Женщина</button></div></div>'
  );
}
...
function showForm() {
  const gender = $('.form__user-gender');
  const birth = $('.form__user-birth');
  const allergy = $('.form__user-allergy');

  $('.user__data').each(function (index, value) {
    if (index === 0) {
      gender.value = $(value).text();
    }
    if (index === 1) {
      birth.value = $(value).text();
    }
    if (index === 2) {
      allergy.value = $(value).text();
    }
  });
...
function addDate() {
  ...
  $('.chat__time>p').html($.date());
}

function addFieldAboutValidity() {
  $('#input-phone').blur(function () {
    if (validatePhone()) {
      $('#spnPhoneStatus').html('Valid');
      $('#spnPhoneStatus').css('color', 'green');
      $('#spnPhoneStatus').on('change', function () {
        $('.submit-roulette').prop("disabled", false);
      });
    }
    else {
      $('#spnPhoneStatus').html('Invalid');
      $('#spnPhoneStatus').css('color', 'red');
      $('.submit-roulette').on('click', function () {
...
function validatePhone() {
  let a = document.getElementById('input-phone').value;
  const regex = /^(\+38)?[\s\-]?\(?[0-9]{3}\)?[\s\-]?[0-9]{3}[\s\-]?[0-9]{2}[\s\-]?[0-9]{2}$/;
  if (regex.test(a)) {
    $('.submit-roulette').attr("disabled", false);
    return true;
  }
  else {
    return false;
...
function showComment() {
  ...
  const glowBtn = $('.glow-button');

  glowBtn.on('click', function () {
    glowBtn.hide(500);
    if (commentItem.className !== 'comment-item-show') {
      commentItem.addClass('comment-item-show');
    }
  });
}